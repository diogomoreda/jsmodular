var Ajax = (function()
{
	...
	var request = function (url, vars, success_callback, fail_callback, method, parseJson)
	{
		...
		p('Ajax.request({url : '+url+' ,vars : '+parameters+' ,success_callback : '+(typeof success_callback)+' ,fail_callback : '+(typeof fail_callback)+' ,method : '+method+' ,parseJson : '+parseJson+'});');
		
		// AJAX onreadystatechange handler
		ajax.onreadystatechange = function()
		{
			p('Ajax.onreadystatechange() -> readyState='+ajax.readyState+'  '+'status='+ajax.status+'  '+'response='+ajax.responseText );
			...
			try
			{
				if (ajax.status	== 200)
				{
					if (ajax.readyState == 4) 
					{
						if(!parseJson)
						{
							p('Ajax response success = '+ajax.responseText);
							return success_callback(ajax.responseText);
						}else{
							try
							{
								var results = JSON.parse(ajax.responseText);
								p('Ajax json parse success = '+results);
							}
							catch(e)
							{
								p('Sorry, Json Parsing failed '+e+' '+ ajax.responseText);
					...
					if (ajax.readyState	== 4 ) 
					{
						p('Sorry, a server error[2] was found '+ajax.status);
			...
			{
				p('Sorry, a server error[1] was found '+e);
